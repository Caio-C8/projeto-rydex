...
export const TrackingProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const enviarLocalizacaoAPI = async (lat: number, lon: number) => {
    try {
      console.log("📍 Enviando localização atualizada para API...", lat, lon);
  ...
  useEffect(() => {
    ...
    const startTracking = async () => {
      // Se estiver offline, para o rastreamento
      if (!isOnline) {
      ...
      // 1. Envio Imediato ao ficar Online (Requisito do usuário)
      ...
      locationSubscription.current = await Location.watchPositionAsync(
        {
          ...
          distanceInterval: 10, // Recebe updates a cada 10 metros no app para verificar a lógica
        ...
        (newLoc) => {
          ...
          // Verifica se já enviamos alguma vez
          ...
          // Se distanciou > 100m OU passou > 5 minutos
  ...
  // Ação de Toggle (Substitui a lógica que estava no useHomeLogic)
