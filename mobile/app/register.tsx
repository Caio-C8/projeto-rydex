...
import * as Location from "expo-location"; // 1. Importação da Localização
...
export default function RegisterScreen() {
  ...
  const formatDataToISO = (dateStr: string) => {
    // Garante que pegamos apenas os números, independente se tem barra ou não
    const numbers = dateStr.replace(/\D/g, "");

    // Se tiver 8 dígitos (ddmmaaaa), converte
    if (numbers.length === 8) {
    ...
    // Retorna a string original se estiver incompleta
    return dateStr;
  };
  ...
  const handleSubmit = async () => {
    ...
    const requiredFields = [nome, cpf, email, password, dataNasc];
    ...
    // Inicia o loading (para cobrir o tempo de pegar a localização)
    setIsLoading(true);

    try {
      // 3. Captura de Localização Atual
      let latitude = 0;
      let longitude = 0;
      ...
      try {
        ...
        if (status === "granted") {
          // getCurrentPositionAsync pode demorar um pouco
          const location = await Location.getCurrentPositionAsync({});
          latitude = location.coords.latitude;
          longitude = location.coords.longitude;
        } else {
          // Opcional: Avisar que a localização é importante, mas não bloquear o cadastro
          console.warn("Permissão de localização negada no cadastro.");
        }
      } catch (locError) {
        console.log("Erro ao obter localização:", locError);
        // Segue o cadastro mesmo sem localização (mandará 0 ou undefined)
      }

      // 4. LIMPEZA DE DADOS (Remove formatação visual)
      const cpfLimpo = cpf.replace(/\D/g, "");
      const celularLimpo = celular.replace(/\D/g, "");

      // 5. Chamada ao serviço com Coordenadas
      await entregadoresService.criarEntregador(
        {
          ...
          // Envia latitude/longitude se foram capturados
