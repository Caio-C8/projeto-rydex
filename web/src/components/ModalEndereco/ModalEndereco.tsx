...
const CliqueNoMapa = ({ aoClicar }: { aoClicar: (lat: number, lng: number) => void }) => {
  useMapEvents({
    click(e) {
...
export const ModalEndereco: React.FC<ModalEnderecoProps> = ({ onFechar, onConfirmar }) => {
  ...
  const handleMapClick = async (lat: number, lng: number) => {
    ...
    try {
      ...
      if (res.data && res.data.address) {
        ...
        setEnderecoDetectado(`${novoEndereco.logradouro}, ${novoEndereco.numero || 'S/N'} - ${novoEndereco.bairro}`);
      } else {
        setEnderecoDetectado("Endereço não identificado neste ponto. Tente outro local.");
      ...
      <div className="modal-mapa-content">
        ...
        <div className="mapa-footer">
          <div className="info-selecao">
            ...
            <p>{enderecoDetectado || "Clique no mapa para marcar onde é a empresa"}</p>
          </div>
